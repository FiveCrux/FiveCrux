...
export function GiveawayDetailClient({
  ...
  const giveawayId = (id ?? (params.id as string)) as string
  ...
  const [giveaway, setGiveaway] = useState<any>(initialData ?? null)
  ...
  useEffect(() => {
    const fetchRelatedGiveaways = async () => {
      ...
      try {
        ...
        // No fallback — leave the section empty so it shows its own empty state.
        setRelatedGiveaways([])
      } finally {
        clearTimeout(t)
