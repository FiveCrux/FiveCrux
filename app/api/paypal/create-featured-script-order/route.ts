...
// POST - Create PayPal order for featured script slots
export async function POST(request: NextRequest) {
  try {
    ...
    const { packageId, durationWeeks, price, slotsToAdd } = body
    ...
    if (![1, 2, 4, 8].includes(durationWeeks)) {
      return NextResponse.json(
        { error: "Invalid duration. Must be 1, 2, 4, or 8 weeks" },
        { status: 400 }
      )
    }
      ...
      purchase_units: [
        {
          ...
          description: `${packageId.toUpperCase()} Featured Script Package - ${slotsToAdd} slot(s) for ${durationWeeks} week(s)`,
          custom_id: JSON.stringify({
            packageId,
            durationWeeks,
            slotsToAdd,
            userId: (session.user as any).id,
            type: "featured_script_slots", // Identify this as featured script slots
          }),
        },
      ],
      application_context: {
