...
export function CategoryClient({
  ...
  initialCategories?: { slug: string; name: string; isActive?: boolean }[]
  slug: string
}) {
  const params = useParams()
  const categorySlug = slug || (params.slug as string)

  const hasServerSeed = Array.isArray(initialScripts) && Array.isArray(initialCategories)
  // Consumed once — a client-side nav to a DIFFERENT category slug still
  // needs a real fetch (we only have seed data for the slug rendered on the
  // server), so this only skips the very first effect run.
  ...
  const initialKnownSlugs = useMemo(
    () => (Array.isArray(initialCategories) ? initialCategories.filter((c) => c.isActive !== false).map((c) => String(c.slug).toLowerCase()) : []),
    [initialCategories]
  )
  const [knownSlugs, setKnownSlugs] = useState<string[]>(initialKnownSlugs)
  const [scripts, setScripts] = useState<Script[]>(() =>
    hasServerSeed ? filterByCategory(initialScripts as Script[], categorySlug, initialKnownSlugs) : []
  )
  const [loading, setLoading] = useState(!hasServerSeed)
  // Dynamic category name from the DB (falls back to the static map / slug).
  const [dbCategoryName, setDbCategoryName] = useState<string | null>(() => {
    const m = Array.isArray(initialCategories)
      ? initialCategories.find((c) => c.slug?.toLowerCase() === categorySlug?.toLowerCase())
      : null
    return m?.name ?? null
  })

  useEffect(() => {
    if (hasServerSeed && !seedConsumedRef.current) {
  ...
  useEffect(() => {
    ...
    const fetchScripts = async () => {
      ...
      try {
        ...
        setScripts(filterByCategory(data.scripts || [], categorySlug, knownSlugs))
      } catch (error) {
